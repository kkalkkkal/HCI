...
public class CommonAdapter extends RecyclerView.Adapter<CommonAdapter.ViewHolder> implements Serializable {
    ...
    public void onBindViewHolder(@NonNull @NotNull ViewHolder holder, int position) {
        ...
        holder.addCompareBtn.setOnClickListener((v) -> {
            ...
            //update ui
            if (isAddedCompareList(school)) {
                Toast.makeText(context, "비교 리스트에 추가되었습니다", Toast.LENGTH_SHORT).show();
                holder.addCompareBtn.setColorFilter(context.getColor(R.color.yellow));
            } else {
                Toast.makeText(context, "비교 리스트에서 제거되었습니다", Toast.LENGTH_SHORT).show();
                holder.addCompareBtn.setColorFilter(context.getColor(R.color.gray_light));
            }
//            try {
...
//            }
    ...
    private boolean isAddedCompareList(School school) {
        return CompareSchoolManager.Companion.contains(school);
//        try {
//            File dir = new File(context.getFilesDir().toString());
//            File f = new File(dir, "UserDB.xls");
//
